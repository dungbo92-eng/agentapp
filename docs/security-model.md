...
- Windows DPAPI로 암호화한 local credential vault
- worker별 session profile descriptor와 local runtime 상태
...
dashboard에서 password/API key를 입력받는 경우, 값은 repo에 쓰지 않고 `data/credential-vault.json` 또는 desktop 앱의 `userData/data/credential-vault.json`에 Windows DPAPI로 암호화해 저장한다.

- 암호화는 현재 Windows 사용자 계정 경계에 묶인다.
- `data/dashboard-runtime.json`에는 `credentialRef`, `credentialStatus` 같은 metadata만 남긴다.
- handoff, memory, plan, snapshot, 로그에는 secret 원문과 ciphertext를 남기지 않는다.
- vault는 worker adapter가 정상 인증 흐름을 시작할 때만 참조한다.
- MFA, captcha, OAuth consent, 플랫폼 승인창이 뜨면 자동 처리하지 않고 사용자에게 넘긴다.
...
허용되는 자동 선택은 `enabled=true`, `sessionStatus=ready`인 로컬 session profile 중에서 작업 난이도와 예산에 맞는 profile을 고르는 것이다. 실제 인증 세션이 만료되었거나 추가 보안 확인이 필요한 경우 자동 선택은 멈추고 handoff에 남긴다.
