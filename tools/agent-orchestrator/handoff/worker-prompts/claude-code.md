...
- Use `pnpm agent:route -- --task "OS 작업 스케줄러 또는 Codex automation 연동 방식 정리" --provider claude` before expensive reasoning work.
...
```bash
pnpm agent:route -- --task "OS 작업 스케줄러 또는 Codex automation 연동 방식 정리"
```
