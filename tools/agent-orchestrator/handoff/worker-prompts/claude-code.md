...
- Dashboard adapter runs `claude --print` with a session-profile-specific CLAUDE_CONFIG_DIR.
- Permission mode defaults to acceptEdits so local edits do not require interactive approval.
- If the session profile is not authenticated yet, run `claude` once in that profile and mark the account Ready again.
...
- Use `pnpm agent:route -- --task "Claude/Gemini CLI 설치 후 실제 authenticated cycle 재검증" --provider claude` before expensive reasoning work.
