...
- Generated: 2026-05-09T12:54:24.694Z
- Selected task: 계정 수/요금제/남은 주간 사용량 설정 스키마 작성
- Selection source: task-queue
- Task id: usage-budget-schema
- Task priority: 95
...
1. 계정 수/요금제/남은 주간 사용량 설정 스키마 작성.
2. 작업 난이도별 모델 추천 CLI 초안 작성.
3. `agent-progress`가 phase별 진행률을 출력하도록 개선.
4. `agent-report`가 project_state까지 갱신하도록 개선.
...
auto_allowed:
  ...
  - id: usage_budget_planning
    description: "Track user-provided remaining weekly usage units and recommend model/account allocation without storing credentials."
    limits:
...
```json
{
  ...
  "tasks": [
    ...
    },
    {
      ...
      "priority": 60,
      "status": "pending",
      "depends_on": ["agent-next-priority"],
      "blocked_by": [],
      "reason": "실행 전에 allow/hold/deny 분류를 확인해야 한다."
    },
    {
      "id": "worker-prompt-templates",
      "title": "worker별 프롬프트 템플릿 생성 명령 추가",
      "phase": "phase-2",
      "priority": 55,
      "status": "pending",
      "depends_on": ["agent-next-priority"],
      "blocked_by": [],
      "reason": "Codex, Claude Code, Cursor, Gemini CLI별 handoff 시작을 표준화한다."
    },
    {
      "id": "dashboard-decision-screen",
      "title": "진행률/다음 작업/보류 결정 화면",
      "phase": "phase-5",
      "priority": 40,
      "status": "pending",
      "depends_on": ["agent-progress-by-ph
