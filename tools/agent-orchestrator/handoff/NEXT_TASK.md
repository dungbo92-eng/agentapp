...
- Generated: 2026-05-09T13:06:30.296Z
- Selected task: 사용량 부족 시 작업 분해 또는 사용자 결정 요청 handoff 구현
- Selection source: task-queue
- Task id: usage-insufficient-handoff
- Task priority: 80
...
1. 사용량 부족 시 작업 분해 또는 사용자 결정 요청 handoff 구현.
2. `agent-progress`가 phase별 진행률을 출력하도록 개선.
3. `agent-report`가 project_state까지 갱신하도록 개선.
4. 안전 작업/보류 작업을 분류하는 dry-run 명령 추가.
5. worker별 프롬프트 템플릿 생성 명령 추가.
