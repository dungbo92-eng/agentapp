...
- Status: completed
- Summary: dashboard 계정 설정을 고정 조합에서 동적 Add account 흐름으로 전면 교체했다. provider/login method/email/session profile/password/API key 입력을 지원하고, secret은 Windows DPAPI local vault에 암호화 저장하며 runtime에는 crede
...
auto_allowed:
  ...
  - id: local_encrypted_credential_vault
    ...
    limits:
      ...
      - "never store plaintext credentials in repo, logs, memory, handoff, MCP configs, or dashb
