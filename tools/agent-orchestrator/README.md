...
plugin/MCP 확장 전략은 `../../docs/plugin-mcp-extension-strategy.md`를 따른다. 새 도구는 local read, local write, external read, external write, denied 계층으로 분류하고 worker prompt와 handoff에 사용 결과를 남긴다.

제품화 테스트 시나리오는 `../../docs/test-scenarios.md`를 따른다. 새 PC 시작, handoff 이어받기, 예산 라우팅, 보류 결정, scheduled check, dashboard, git sync를 최소 smoke set으로 확인한다.
