...
스크립트: [`scripts/integrate-ponytail.mjs`](../../../../scripts/integrate-ponytail.mjs) — 프리앰블 미리보기(출력만).

**프로덕션 적용됨 (DEC-20260619-001=A)**: `dashboard-runtime.mjs`의 `applyPonytailPreamble(prompt, mode)`가 `settings.integrations.ponytailMode`(off/lite/full)에 따라 worker 프롬프트 앞에 룰을 1회 prepend한다(멱등, `[PONYTAIL 규칙]` 마커). 룰 단일 소스는 `ponytail.rule.md`이며 `tools/**`로 패키징된다. dashboard 설정에서 모드를 고른다. 회귀 검증: `scripts/validate-integrations.mjs`.
