...
- Updated: 2026-06-20
- Status: completed
- Summary: codebase-memory-mcp + Ponytail을 평가→PoC/dry-run→프로덕션 wiring까지 적용. worker-launch-adapter MCP 등록 + dashboard-runtime Ponytail 프리앰블 + dashboard 설정 토글 + validate-integrations 회귀 검증을 추가했다. DEC-20260619-001=A(contained) 채택, 기본 off.
- Verification: pnpm dashboard:build 통과; node scripts/validate-integrations.mjs 11/11 통과; node --check (runtime/adapter); codebase-memory-mcp cli index/architecture/search
- Git: commit + main push + desktop 릴리즈
- Decisions: DEC-20260619-001 resolved (Option A)
- Next: 인증 세션에서 MCP 활성 worker 실 사이클 검증
