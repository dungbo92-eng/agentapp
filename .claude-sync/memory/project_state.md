...
1. Claude Code command-mode adapter profile.
2. Gemini CLI command-mode adapter profile.
3. tool별 login/session-expired detector 보강.
4. doctor에 session profile readiness 진단 추가.

## 열린 질문

- Claude Code CLI가 설치된 환경에서 session profile 홈을 어떤 경계로 분리할지 확인이 더 필요하다.
- Gemini CLI의 session/config 디렉터리 분리 방식은 실제 설치 환경 기준 확인이 필요하다.

## 최근 보고

- Updated: 2026-05-10T14:17:20.025Z
- Status: completed
