...
- Updated: 2026-07-10
- Status: completed
- Summary: **직전 세션 세션한도 중단 이어받기.** 인앱 터미널 `Cannot find module '../build/Release/conpty.node'` 근본원인=순수 패키징 버그(electron-builder 가 node-pty 의 `build/Release/*.node` 를 node_modules 수집에서 드롭, `build.files` 화이트리스트에 명시 포함 없음). node-pty 는 N-API 기반이라 ABI 무관→리빌드 불요, 바이너리만 실으면 됨. 수정: `build.files` 에 node-pty 명시 포함(+`.pdb`/`.map`/`src`/`deps` 제외 경량화 8.4M), 신규 `scripts/after-pack.cjs` 가드(unpacked 에 conpty/pty.node 없으면 빌드 실패→깨진 터미널 재릴리즈 차단), 터미널 기본 cwd `app.getPath("home")` 폴백. 하드코딩 경로는 이미 포터블(userData/import.meta.url/스냅샷 치환) 확인, RC 폰 미표시는 프로젝트 `remoteControl=false` 토글 상태 문제(코드 아님).
- Verification: `electron-builder --dir --win --x64` 실측 2회 → afterPack 통과 + unpacked 에 conpty.node(312832)/pty.node/winpty-agent.exe/winpty.dll 확인, .pdb 0; pnpm validate 통과(runtime-race 50/50, e2e 7/7); node --check 전체.
- Git: commit + main push + desktop 릴리즈(patch)
- Next: 자동업데이트 후 인앱 터미널 실행 확인; RC 원하면 프로젝트 📱 토글 on.
