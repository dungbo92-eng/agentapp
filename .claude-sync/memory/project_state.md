...
- Updated: 2026-06-20
- Status: completed
- Summary: Claude Remote Control 을 **숨긴 콘솔 방식으로 실제 작동**하게 붙임(node-pty 폐기). 근본원인=node-pty win32 prebuilt 부재로 pty 크래시. `spawnRemoteControlConsole`(cmd /c claude --remote-control, detached+windowsHide → 창 없이 TTY)로 시작 시 ready Claude 계정마다 자동 실행, taskkill /T 정리. 계정 카드에 세션 상태등(🟢/🟡/🔴) + 실행 중 📡 RC. **실계정 2개 E2E로 12초 생존 확인**.
- Verification: pnpm dashboard:build 통과; validate-remote-control 10/10; node --check 전체; 실계정 spawn 2/2 생존
- Git: commit + main push + desktop 릴리즈(patch)
- Decisions: DEC-20260619-001 resolved (Option A)
- Next: 업데이트+재시작 후 폰 Claude 앱에서 계정별 remote-control 세션 확인
