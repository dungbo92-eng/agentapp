...
- Updated: 2026-06-20
- Status: completed
- Summary: Claude Remote Control 붙임 — 앱 시작 시 ready Claude 계정마다 node-pty 로 `claude --remote-control <계정명>` 자동 실행, 폰(Claude 앱/웹)에서 원격 조종(LAN·방화벽 무관). `remoteControlAutoStart`(기본 on), `listReadyClaudeAccounts`, `buildRemoteControlSpec`, IPC get/start/stop, dashboard 원격제어 패널, validate-remote-control(9). 직전 사이클의 LAN auto-rc(autoRcOnSession)는 사용자 정정으로 **전부 revert**(RC=`claude --remote-control` 이었음). 수동 모바일 접속 토글은 유지.
- Verification: pnpm dashboard:build 통과; validate-remote-control 9/9 + validate-integrations 11/11 (pnpm validate 체인 내); node --check 전체 통과
- Git: commit + main push + desktop 릴리즈
- Decisions: DEC-20260619-001 resolved (Option A)
- Next: 업데이트 후 폰 Claude 앱에서 계정별 remote-control 세션이 잡히는지 실제 확인
