...
- Updated: 2026-06-20
- Status: completed
- Summary: 설치 앱 시작 크래시(`conpty.node` 로드 실패) 수정 + 계정 상태등 추가. v0.15.0 의 remote-control PTY 자동실행이 시작 시 node-pty 를 로드하다 패키징 asar 에서 conpty.node 미해결로 실패 → remote-control 기능(자동실행/패널/IPC/buildRemoteControlSpec/listReadyClaudeAccounts/validate-remote-control) **전부 제거**. 대신 왼쪽 계정 카드에 세션 상태등(ready=🟢/paused=🟡/needs-login=🔴). 원격제어는 사용자가 터미널에서 `claude --remote-control` 직접 실행.
- Verification: pnpm dashboard:build 통과; validate-integrations 11/11 (pnpm validate 내); node --check 전체 통과; dangling ref 0
- Git: commit + main push + desktop 릴리즈(patch)
- Decisions: DEC-20260619-001 resolved (Option A)
- Next: (후속) 패키지 conpty.node 해결하면 in-app 터미널/원격제어 자동실행 부활 가능
