...
- Updated: 2026-07-09
- Status: completed
- Summary: RC 폰 미표시 **근본원인 규명·수정** + 프로젝트별 모바일 세션 토글. 원인=미신뢰 폴더에서 interactive `claude --remote-control` 이 "이 폴더 신뢰?" 대화상자에 (숨겨진 채) 멈춰 RC 등록 실패(폰 실측 A/B/C 확정). 수정: `ensureClaudeFolderTrusted`+`normalizeClaudeCwd` 로 spawn 전 폴더를 프로필 `.claude.json` 에 사전 신뢰(키 형식 claude 실제값과 일치 검증). arg 견고화(`quoteWindowsArg` — 공백 인자 안 깨짐). 프로젝트 토글: `remoteControl`(기본 on)+`updateProject`+`/projects/update`+UI 📱 토글, `listRemoteControlTargets` 가 켠 프로젝트만. + 세션 배정은 **계정×프로젝트 교차곱**(각 계정이 프로젝트마다 세션, 계정2×프로젝트2=4세션).
- Verification: node --check; validate-remote-control 31/31; pnpm validate(runtime-race 사전 플래키만 FAIL); dashboard:build 통과; **폰 실측**(dungdy92): 신뢰폴더 뜸/미신뢰 안 뜸/사전신뢰 다시 뜸.
- Git: commit + main push + desktop 릴리즈(minor)
- Decisions: none
- Next: 업데이트+재시작 후 폰에서 프로젝트별 세션 접근 + 토글 off 동작 확인.
