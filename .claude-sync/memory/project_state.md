...
- Updated: 2026-07-10
- Status: completed
- Summary: **RC "연결완료인데 폰에 안 뜸" 의 마지막 false-ready 경로 제거 + 보이는 터미널 RC 매크로.** `detectAccountSession` 이 `credentialStatus === "stored"` 면 세션 검사를 건너뛰고 즉시 `ready` 를 반환하고 있었다. vault 의 비밀번호/API 키는 로그인 창 자동입력용일 뿐 CLI 인증에 주입되지 않는다(grep 으로 확인) → 한 번도 로그인 안 한 계정이 초록불 + `📡 RC`, hidden 콘솔은 로그인 프롬프트에서 멈춤. v0.17.3 토큰 검사가 이 분기 뒤라 무력화돼 있었다. 우회 제거로 실측 dungbo92(빈 stub 토큰) → `needs-login` 정정. 추가로, 3중 프롬프트 중 **OAuth 로그인은 숨긴 콘솔로 절대 못 넘기므로** 계정 카드에 `📱 RC 터미널` 매크로 신설 — 숨긴 spawn 과 같은 CLAUDE_CONFIG_DIR/cwd/args 를 PowerShell 한 줄(`buildRemoteControlTerminalCommand`)로 만들어 보이는 사이드 터미널에 타이핑, 사용자가 프롬프트를 직접 보고 응답한다. terminal-create 가 `env`/`initialCommand`/`shell` 지원.
- Verification: 실데이터 detectAccountSession → dungbo92 needs-login / dungdy92 ready, listReadyClaudeAccounts 2→1; 터미널 명령 실계정 생성 + `O''Brien` 이스케이프 확인; validate-remote-control 32/32(신규 11); pnpm validate exit 0; dashboard:build 통과.
- Git: commit + main push + desktop 릴리즈(minor)
- Next: `📱 RC 터미널` 로 dungbo92 로그인 완료 → 폰 세션 등록 확인.
