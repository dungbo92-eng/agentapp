...
- Updated: 2026-07-09
- Status: completed
- Summary: RC(remote-control) 창 숨김 + 프로젝트 경로별 세션. 창 뜨던 근본원인=`detached`(DETACHED_PROCESS)로 자식이 콘솔 못 받아 claude 가 AllocConsole 로 보이는 창 생성. → `spawnRemoteControlConsole` 을 **PowerShell `Start-Process -WindowStyle Hidden -PassThru`** 로 교체(stdio 리다이렉트 안 함 → 실제 콘솔 TTY 확보 = 모바일 유지, 창은 숨김, -PassThru 로 실 PID 받아 tree-kill). `listRemoteControlTargets()` 로 **등록 프로젝트 경로마다 1세션**(경로 4개면 4세션, ready Claude 계정 라운드로빈, cwd=프로젝트 경로), 없으면 계정당 폴백. main.mjs 키 accountId::projectId, isPidAlive liveness. UI 배지 `📡 RC ×N` + 프로젝트 tooltip.
- Verification: node --check 3파일; validate-remote-control 21/21; pnpm validate(runtime-race 사전 플래키만 FAIL, integrations 11 통과); dashboard:build 통과; hidden Start-Process 실측 PID 캡처+생존(창 안 뜸).
- Git: commit + main push + desktop 릴리즈(minor)
- Decisions: none
- Next: 업데이트+재시작 후 폰에서 프로젝트별 RC 세션 접근 확인. (실 claude+폰 end-to-end 는 사용자 확인 필요.)
