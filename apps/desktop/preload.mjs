...
contextBridge.exposeInMainWorld("agentapp", {
  ...
  // Claude Remote Control — 계정별 `claude --remote-control` 세션. 상태 조회 + 수동
  // 시작/중지. 폰(Claude 앱/웹)에서 각 계정 세션을 원격 조종한다.
  remoteControl: {
    ...
    onData: (handler) => onChannel("agentapp:remote-control-data", handler),
    onExit: (handler) => onChannel("agentapp:remote-control-exit", handler),
  },
